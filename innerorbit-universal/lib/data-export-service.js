...
export async function completeDataDeletion(userId, auth) {
    try {
        ...
        // 1. Get conversations and profile in parallel
        ...
        // 2. Build and execute all deletion tasks in parallel
        const deletePromises = [
            ...
            (async () => {
                try {
                    ...
                    Logger.error('[DataDeletion] Error deleting user profile:', error);
                }
            })()
        ];
        ...
        if (auth.currentUser && auth.currentUser.uid === userId) {
            try {
                ...
                authDeleteError = err;
                Logger.error('[DataDeletion] Failed to delete Firebase Auth account:', err);
                // Force sign out so they aren't stuck in a zombie state
                try {
                    await auth.signOut();
                } catch (e) { }
            }
        ...
        if (authDeleteFailed) {
            throw new Error('Data cleared, but Auth account requires recent login to delete: ' + authDeleteError.message);
        }
