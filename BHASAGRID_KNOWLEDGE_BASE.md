# 🛡️ BhasaGrid Comprehensive Knowledge Base

This document serves as the definitive technical guide for the BhasaGrid messaging ecosystem. It is based on a structural and semantic analysis of the codebase (May 2026) and reflects the completion of the **v5.5 Hybrid Identity Model & Auth UX Hardening** phase.

---

## 1. Executive Overview

BhasaGrid is a multi-layered, privacy-first messaging platform designed for maximum resilience against both conventional and quantum-scale surveillance. It features a "Stealth UI" entry point and a cascading encryption engine.

- **Primary Entry**: CalcX (Calculator Stealth Interface only for mobiles)
- **Core Engine**: `bhasagrid-universal` (Cross-platform TS/JS)
...
BhasaGrid is architected as a **Hybrid of Signal and WhatsApp**:
...
BhasaGrid implements a **Zero-Knowledge 3-Layer pipeline** for high-resolution 4K media.
...
BhasaGrid employs a physical memory erasure strategy to ensure sensitive credentials never persist in the JS heap:
...
BhasaGrid uses a **Hybrid Loading Strategy** to distinguish between security-critical operations and background data tasks:
...
- `bhasagrid-universal/scripts/firebase-config.js` — loads from env vars only ✅
- `bhasagrid-universal/lib/firebase.js` — reads from `Constants.expoConfig.extra` / env vars ✅
...
## 9. Infrastructure & Ownership Migration Status (May 2026)

### 👤 Ownership Profile
* **Founder**: Sreyan Dash
* **Primary Business Account**: [bhasagrid.dev@gmail.com](mailto:bhasagrid.dev@gmail.com)
* **Custom Domain**: `bhasagrid.in`
* **Development Phase**: Pre-launch, small-scale testing (~5 authentication users, 2–3 active testers).

### 🌐 Infrastructure & Accounts
* **Hostinger Ownership**: Fully migrated to [bhasagrid.dev@gmail.com](mailto:bhasagrid.dev@gmail.com).
* **Firebase Console Access**: Full Owner access granted to [bhasagrid.dev@gmail.com](mailto:bhasagrid.dev@gmail.com).
* **GitHub Audit**: Security reviewed; no exposed secrets. Recommendations include setting up a GitHub Organization (`BhasaGrid`) and migrating personal repositories.

### 🛢️ Firebase Coexistence & Migration Plan
To ensure total data protection, BhasaGrid maintains a dual-project strategy during the pre-launch stabilization phase:

1. **Legacy Active Project** (`innerorbit-bc8ce` / `innerorbit-portal`):
   * Serves active developer data, authentication databases, and download portal hosting.
   * **Do not delete or decommission** under any circumstances during active feature stabilization.
2. **New Clean Identity Project** (`bhasagrid`):
   * Created as a clean slate under the new BhasaGrid identity.
   * Reserved for future stable infrastructure migrations once all core features (Authentication, Chat, Calls, Presence) have reached absolute production stability.
3. **Migration Sequence**:
   * Complete all feature stabilizations ➔ Verify Android releases ➔ Draft detailed data migration scripts (users, publicProfiles, conversations) ➔ Perform staging runs to `bhasagrid` project ➔ Safely direct custom domains to new Firebase project hosting.
