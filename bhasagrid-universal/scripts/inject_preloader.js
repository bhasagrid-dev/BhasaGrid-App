...
const logoPath = path.join(rootDir, 'assets/BhasaGrid-Logo.png');
...
// Key 'bhasagrid-app-storage' is set by themeStore.js persist middleware.
// Priority: saved preference → OS preference → dark (fallback).
function getPreloaderTheme() {
    try {
        const raw = localStorage.getItem('bhasagrid-app-storage');
        if (raw) {
