...
function RootLayoutNav() {
  ...
  const { user, loading: authLoading, isLoggingOut, isDeletingAccount, isDecoyMode, isUnlocking, isBiometricLocked, welcomeData, hasSessionHint } = useAuth();
  ...
  useEffect(() => {
    if (prevUser.current && !user && !isLoggingOut && !isDeletingAccount) {
      if (isWeb) setShowExpiredModal(true);
    }
    prevUser.current = user;
  }, [user, isLoggingOut, isDeletingAccount]);
  ...
  useEffect(() => {
    if (isReady && !isLoggingOut && !isDeletingAccount) {
  ...
  }, [isReady, isLoggingOut, isDeletingAccount]);
  ...
  useEffect(() => {
    if (!isReady || !isSettled || isLoggingOut || isDeletingAccount) {
      return;
    }
    ...
    const routingGuard = async () => {
      ...
      if (isWeb && typeof sessionStorage !== 'undefined') {
        const stored = sessionStorage.getItem('__BHASAGRID_UNLOCK_GRACE_PERIOD');
        if (stored) persistentGrace = parseInt(stored, 10);
      }
      const isWithinGracePeriod = (globalThis.__BHASAGRID_UNLOCK_GRACE_PERIOD && now < globalThis.__BHASAGRID_UNLOCK_GRACE_PERIOD) ||
    ...
    routingGuard();
  }, [user, segments, isReady, isSettled, isLoggingOut, isDeletingAccount, isUnlocking, isBiometricLocked, isDecoyMode, welcomeData]);
