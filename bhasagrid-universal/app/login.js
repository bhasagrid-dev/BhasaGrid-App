...
const LOGO_IMG = require("../assets/BhasaGrid-Logo.png");
...
export default function LoginScreen() {
  ...
  // Project ID: bhasagrid-bc8ce
  // Console: https://console.cloud.google.com/apis/credentials?project=bhasagrid-bc8ce
  ...
  useEffect(() => {
    if (response) {
      ...
      if (response.type === "success") {
        ...
        // 🛡️ Resume auto-lock after cancel/error
        setPermissionRequesting(false);
      }
    }
  }, [response]);
  ...
  const handleMobileGoogleLogin = async (idToken) => {
    try {
      ...
      // 🛡️ Always resume auto-lock after mobile Google auth completes
      setPermissionRequesting(false);
    }
  };
  ...
  const handleBiometricAuth = async () => {
    try {
      const result = await LocalAuthentication.authenticateAsync({
        promptMessage: 'Unlock BhasaGrid',
  ...
  const handleGoogleSignIn = async () => {
    if (isWeb) {
      ...
      // Mobile Logic
      // 🛡️ CRITICAL: Pause auto-lock before opening Google browser.
      // promptAsync() opens a Chrome Custom Tab which backgrounds the app.
      // Without this, the 2s stealth lock timer fires and re-engages Calculator UI.
      ...
      >
        ...
        >
          <Animated.View style={{
              ...
              <View style={[styles.footer, { marginTop: 12 }]}>
                <View style={{ alignItems: 'center', gap: 12 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <Text style={[styles.footerText, { fontSize: isCompact ? 13 : 15 }]}>New to BhasaGrid? </Text>
            ...
            {!isWeb && <Pressable
              onPress={() => Linking.openURL('https://bhasagrid-portal.web.app/')}
              style={({ pressed }) => ({
                marginTop: 16,
                alignSelf: 'center',
                flexDirection: 'row',
