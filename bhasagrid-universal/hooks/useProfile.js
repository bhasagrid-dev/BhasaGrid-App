  ...
  useEffect(() => {
    if (user?.uid) {
      ...
      firestoreService.getUserProfile(user.uid)
        .then(async (profile) => {
          if (profile) {
            // 🛠️ SELF-HEALING: Profile doc exists but userId/pin are missing.
            // This happens when publishCapabilities or updateUserProfile created the doc
            // with only encryption fields, but createUserProfile was interrupted (e.g. Metro
            // hot reload, process death, or stealth re-lock during Google Sign-In).
            if (!profile.userId) {
              Logger.warn(`[useProfile] ⚠️ Profile doc exists but userId is MISSING for ${user.uid.substring(0, 8)}. Repairing...`);
              try {
                const newProfile = await firestoreService.createUserProfile(user);
                setMyUserId(newProfile.userId);
                setUserPin(newProfile.pin);

                // 🔐 Also save to hardware storage so "Reveal PIN" works in Account Security
                if (newProfile.userId && newProfile.pin) {
                  await IdentitySecurityService.saveIdentityLocally(newProfile.userId, newProfile.pin);
                }

                Logger.log(`[useProfile] ✅ Profile repaired: ${newProfile.userId}`);
              } catch (err) {
                Logger.error("[useProfile] Profile repair failed:", err);
                showError("Profile repair failed. Please restart.");
              }
              return;
            }
