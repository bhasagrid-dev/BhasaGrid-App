...
export function useHomeActions(ui, user, profile, privacyLevel, isDesktop, setSelectedConversationId, setDesktopDetailView) {
  const { logout, setIsDeletingAccount } = useAuth();
        ...
        { text: "Cancel", onPress: () => setAlertConfig(prev => ({ ...prev, visible: false })) },
        {
          ...
          onPress: async () => {
            try {
              ...
              if (setIsDeletingAccount) setIsDeletingAccount(true);
              await exportService.completeDataDeletion(user.uid, auth);
              setTimeout(() => {
                if (setIsDeletingAccount) setIsDeletingAccount(false);
              }, 1500);
  ...
  const handleAddChatUser = async (targetUser) => {
    ...
    // 🛡️ GUARD: Prevent self-connection
    if (targetUser.uid === user.uid) {
      showError("You can't connect with yourself");
      return;
    }

    // 🛡️ GUARD: Reject corrupt/incomplete profiles
    if (!targetUser.uid) {
      showError("Invalid user profile");
      return;
    }
    ...
    try {
      ...
      const result = await firestoreService.sendConnectionRequest(
        ...
        { userId: targetUser.userId || "Unknown" }
      );
      ...
      } else if (result.status === 'self_request') {
        showError("You can't connect with yourself");
      } else if (result.status === 'rejected_cooldown') {
        showError("This user recently declined your request. Try again later.");
