...
/**
 * User ID Generator Utility for BhasaGrid
...
function isBlockedSequence(id) {
  ...
  // Also block consecutive identical digits (diff is 0) to avoid "double" digits like "22"
  ...
  for (let i = 0; i < digits.length - 1; i++) {
     ...
     if (Math.abs(diff) === 1 || Math.abs(diff) === 9 || diff === 0) {
       return true; // Blocks "12" (diff 1), "21" (diff -1), "09" (diff 9), "90" (diff -9), "22" (diff 0)
     }
  }
