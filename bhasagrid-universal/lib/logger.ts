...
export const Logger: any = {
    ...
    warn: (message: string, ...args: any[]) => {
        if (isDev) {
            ...
            console.warn(`[BhasaGrid] ⚠️ ${message} ${formattedArgs}`.trim());
        }
    },

    // Errors (Red)
    error: (message: string, errorObj: any = null) => {
        let errorMsg = `[BhasaGrid] ❌ ${message}`;
        if (errorObj) {
...
if (isDev) {
    console.log("[BhasaGrid][logger.ts] 🚀 Module loaded successfully");
}
