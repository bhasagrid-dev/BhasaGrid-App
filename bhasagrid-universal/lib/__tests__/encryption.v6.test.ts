  ...
  describe('AsyncStorage integration', () => {
    test('setItem is called on initializeV6Session', async () => {
      ...
      expect(AsyncStorage.setItem).toHaveBeenCalledWith(
        expect.stringContaining('@bhasagrid_ratchet_v6_storage-test'),
        expect.any(String)
      );
    });
    ...
    test('getItem is called on hasV6Session', async () => {
      ...
      expect(AsyncStorage.getItem).toHaveBeenCalledWith(
        expect.stringContaining('@bhasagrid_ratchet_v6_storage-check')
      );
    });
    ...
    test('removeItem is called on deleteV6Session', async () => {
      ...
      expect(AsyncStorage.removeItem).toHaveBeenCalledWith(
        expect.stringContaining('@bhasagrid_ratchet_v6_to-delete')
      );
    });
    ...
    test('v6 sessions use a separate key prefix from v4 sessions', async () => {
      ...
      expect(v6Call[0]).toContain('@bhasagrid_ratchet_v6_');
      expect(v6Call[0]).not.toContain('@bhasagrid_ratchet_prefix-test');
    });
  });
});
