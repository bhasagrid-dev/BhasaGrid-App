    ...
    describe("Ephemeral Key Derivation (Argon2id)", () => {
        test("should derive keys consistently for the same input", async () => {
            ...
            (SecureStore.getItemAsync as jest.Mock).mockImplementation((key) => {
                if (key === "bhasagrid_device_key") return Promise.resolve("mock-device-key");
                if (key === "bhasagrid_device_salt") return Promise.resolve("mock-device-salt");
    ...
    describe("Legacy Key Migration", () => {
        test("should migrate legacy @ keys to new keys", async () => {
            ...
            (AsyncStorage.getItem as jest.Mock).mockImplementation((key) => {
                if (key === "@bhasagrid_device_key") return Promise.resolve("legacy-val");
                return Promise.resolve(null);
            });
            ...
            // Verification: 
            expect(SecureStore.setItemAsync).toHaveBeenCalledWith("bhasagrid_device_key", "legacy-val");
            expect(AsyncStorage.removeItem).toHaveBeenCalledWith("@bhasagrid_device_key");
        });
    });
