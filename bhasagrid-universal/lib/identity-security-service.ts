...
const LOCAL_ENC_PIN = "bhasagrid_enc_pin";
const LOCAL_ENC_USERID = "bhasagrid_enc_userid";
const CLOUD_SYNC_ENABLED = "bhasagrid_cloud_sync_identity";
...
export const IdentitySecurityService = {
  ...
  /**
   * Clears the local identity from hardware storage.
   */
  async clearLocalIdentity(): Promise<void> {
    try {
      await setSecureItem(LOCAL_ENC_PIN, "");
      await setSecureItem(LOCAL_ENC_USERID, "");
      Logger.log("[Identity] 🧹 Local identity cleared from secure storage.");
    } catch (e) {
      Logger.error("[Identity] Failed to clear local identity:", e);
    }
  },
  ...
  async getOrCreateProfileKey(): Promise<string> {
    try {
      ...
      const storedPriv = await getSecureItem("bhasagrid_identity_priv");
      if (!storedPriv) {
      ...
      const salt = "BhasaGrid:Presence:v1";
