...
/**
 * Firestore Service Module for BhasaGrid
...
// 🔒 RESERVED BRAND NAMES: Nobody else can claim these display names.
// These mirror the Firestore security rules — defence-in-depth.
...
// UID of the main exclusive account that is EXEMPT from the restriction.
const EXCLUSIVE_OWNER_UID = "E2Q2rh35cRYZaLkQoTougqbYIG82";

/**
 * Throws a clear error if the displayName is reserved and the caller is not the exclusive owner.
 * @param {string} uid - The UID of the user trying to set the name.
 * @param {string|undefined} displayName - The proposed display name.
 */
function checkReservedDisplayName(uid, displayName) {
  if (!displayName) return;
  const isExclusive = uid === EXCLUSIVE_OWNER_UID || auth.currentUser?.email === "bhasagrid.dev@gmail.com";
  if (isExclusive) return;
  if (RESERVED_DISPLAY_NAMES.includes(displayName.trim().toLowerCase())) {
    throw new Error(
      `The name "${displayName}" is reserved and cannot be used. Please choose a different name.`
    );
  }
}
...
export async function updateUserProfile(uid, updates) {
  ...
  // 🔒 Reject reserved brand names before touching Firestore (defence-in-depth)
  checkReservedDisplayName(uid, updates?.displayName);
...
export async function searchUserByUserId(userId) {
  try {
    const q = query(collection(db, PUBLIC_PROFILES_COLLECTION), where("userId", "==", userId));
    const querySnapshot = await getDocs(q);
...
export async function sendConnectionRequest(senderUid, receiverUid, senderInfo, receiverInfo = null) {
  ...
  // 🛡️ GUARD: Prevent self-connection (would break encryption and corrupt chat list)
  if (senderUid === receiverUid) return { status: 'self_request' };
  ...
  try {
    ...
    if (!snapIn.empty) {
      ...
      // NOTE on argument order: The ORIGINAL pending request was sent FROM receiverUid TO senderUid.
      // So in respondToConnectionRequest(id, status, senderId, receiverId):
      //   senderId  = receiverUid  (the original request's sender)
      //   receiverId = senderUid   (the original request's receiver)
      // This looks swapped relative to the outer function's parameter names, but is correct.
    ...
    // 3. Spam guard: Check if a request was rejected within the last 24 hours
    ...
    const snapRejected = await getDocs(qRejected);
    if (!snapRejected.empty) {
      const lastRejected = snapRejected.docs[0].data();
      const rejectedAt = lastRejected.respondedAt?.toMillis?.() || 0;
      const twentyFourHours = 24 * 60 * 60 * 1000;
      if (Date.now() - rejectedAt < twentyFourHours) {
        return { status: 'rejected_cooldown' };
      }
    }
