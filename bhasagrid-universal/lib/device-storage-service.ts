...
const HARDWARE_LOCK_ENABLED = "bhasagrid_hw_lock_enabled";
const HARDWARE_CREDENTIAL_ID = "bhasagrid_hw_cred_id";
const ENCRYPTED_DEVICE_KEY = "bhasagrid_enc_device_key";
const DEVICE_KEY_STORAGE = "bhasagrid_device_key";
const DEVICE_SALT_STORAGE = "bhasagrid_device_salt";
...
export async function getDeviceKeys(): Promise<{ deviceKey: string; deviceSalt: string }> {
  try {
    ...
    const shard0 = await AsyncStorage.getItem("bhasagrid_shard_0");
    const shard1 = await getSecureItem("bhasagrid_shard_1"); // Stored in SecureStore if mobile
    const shard2 = await AsyncStorage.getItem("bhasagrid_shard_2");
    ...
    if (!deviceKey) {
      ...
      await AsyncStorage.setItem("bhasagrid_shard_0", s0);
      await setSecureItem("bhasagrid_shard_1", s1);
      await AsyncStorage.setItem("bhasagrid_shard_2", s2);
