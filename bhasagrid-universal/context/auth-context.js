...
export function AuthProvider({ children }) {
  ...
  const [sessionHint, setSessionHint] = useState(() => {
    if (isWeb && typeof sessionStorage !== 'undefined') {
      return sessionStorage.getItem('__BHASAGRID_SESSION_HINT') === 'true';
    }
    return false;
  });
  ...
  const handleGoogleUser = async (user, saveLogin) => {
    ...
    globalThis.__BHASAGRID_UNLOCK_GRACE_PERIOD = expiry;
    if (isWeb && typeof sessionStorage !== 'undefined') {
      sessionStorage.setItem('__BHASAGRID_UNLOCK_GRACE_PERIOD', expiry.toString());
    }
    try {
      ...
      // Without this, the "Reveal PIN" button in Account Security returns null.
      ...
      if (isWeb && typeof sessionStorage !== 'undefined') {
        sessionStorage.setItem('__BHASAGRID_SESSION_HINT', 'true');
      }
  ...
  const [isDeletingAccount, setIsDeletingAccountInternal] = useState(false);
  const isDeletingAccountRef = useRef(false);

  const setIsDeletingAccount = (val) => {
    isDeletingAccountRef.current = val;
    setIsDeletingAccountInternal(val);
  };
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      try {
        if (currentUser) {
          ...
          const justLoggedIn = await AsyncStorage.getItem("__BHASAGRID_JUST_LOGGED_IN");
          if (globalThis.__BHASAGRID_JUST_LOGGED_IN || justLoggedIn === 'true') {
            Logger.log("[Auth] 🆕 Explicit login detected in onAuthStateChanged - Resetting session start timestamp");
            await AsyncStorage.setItem(`session_start_${currentUser.uid}`, Date.now().toString());
            globalThis.__BHASAGRID_JUST_LOGGED_IN = false;
            await AsyncStorage.removeItem("__BHASAGRID_JUST_LOGGED_IN").catch(() => {});
          }
          ...
          try {
            ...
            presenceIntervalRef.current = setInterval(async () => {
              if (isLoggingOutRef.current || isDeletingAccountRef.current) return; // Don't ping if logging out or deleting
          ...
          if (isWeb && typeof sessionStorage !== 'undefined') {
            sessionStorage.removeItem('__BHASAGRID_SESSION_HINT');
          }
  ...
  useEffect(() => {
    const subscription = AppState.addEventListener("change", (nextAppState) => {
      ...
      const now = Date.now();
      const graceUntil = globalThis.__BHASAGRID_UNLOCK_GRACE_PERIOD || 0;
      const inGracePeriod = now < graceUntil;

      // Handle Foreground
      if (nextAppState === "active") {
        // 🔧 DEV: Re-establish Metro WebSocket connection (drops when Hermes pauses in background)
        if (__DEV__) { console.log('[BhasaGrid] 📡 Metro reconnect ping — foreground resume'); }
  ...
  const triggerStealthUnlock = async () => {
    ...
    globalThis.__BHASAGRID_UNLOCK_GRACE_PERIOD = Date.now() + 5000;
  ...
  const authenticateBiometrics = async () => {
    try {
      const result = await LocalAuthentication.authenticateAsync({
        promptMessage: 'Authenticate to access BhasaGrid',
  ...
  const signUp = async (email, password) => {
    try {
      ...
      await AsyncStorage.setItem("__BHASAGRID_JUST_LOGGED_IN", "true");
      globalThis.__BHASAGRID_JUST_LOGGED_IN = true;
  ...
  const signInWithGoogle = async (saveLogin = false) => {
    // 🛡️ CRITICAL: Pause the auto-lock timer for the entire Google auth flow.
    // On Android, signInWithPopup opens a Chrome Custom Tab which backgrounds the app.
    // Without this guard, the 2s lock timer fires and re-engages stealth/decoy mode,
    // causing the app to re-render from Calculator UI and lose the auth callback.
    isPermissionRequestingRef.current = true;
    Logger.log("[Auth] 🛡️ Auto-lock paused for Google Sign-In");
    ...
    try {
      ...
      // 🛡️ CRITICAL: Always resume auto-lock after Google auth completes or fails.
      isPermissionRequestingRef.current = false;
      Logger.log("[Auth] 🛡️ Auto-lock resumed after Google Sign-In");
    }
  };
  ...
  const signIn = async (emailOrUserId, passwordOrPin, isPinLogin = false, immediateSave = false) => {
    try {
      ...
      if (isPinLogin) {
        ...
        // Update sequentially to avoid transaction collisions
        const profileData = await createUserProfile(userCredential.user);
        await updateUserProfile(userCredential.user.uid, { hasSetPassword: true });
