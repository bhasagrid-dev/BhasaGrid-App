    ...
    useEffect(() => {
        if (!isWeb) {
            ...
            const subscription = ScreenCapture.addScreenshotListener(() => {
                Alert.alert(
                    "🔒 Privacy Protected",
                    "Screenshots and screen recordings are strictly disabled within BhasaGrid chats to prevent metadata leakage and unauthorized message persistence."
                );
    ...
    return (
        <View style={[{ flex: 1, backgroundColor: chatBgStyle?.color || THEME.background }, containerStyle]}>
            <KeyboardAvoidingView
                behavior={isWeb ? undefined : select({ ios: "padding", default: undefined })}
                style={[{ flex: 1, alignSelf: 'center', width: '100%', maxWidth: 1200 }, !isWeb && !useStealthKeyboard && { paddingBottom: keyboardVisible ? keyboardHeight : 0 }]}
            ...
            >
                ...
                {isSelectionMode ? (
                    ...
                    <BlurView intensity={100} tint={isDark ? "dark" : "light"} style={[
                        styles.inputContainer,
                        {
                            marginBottom: isWeb ? 0 : (keyboardVisible ? 2 : 10),
                            paddingTop: isWeb ? 0 : 12,
                            paddingBottom: isWeb ? 0 : Math.max(insets.bottom || 0, 12),
                            backgroundColor: 'transparent',
                            borderTopWidth: 0,
                            position: isWeb ? 'absolute' : 'relative',
                            bottom: isWeb ? 24 : 0,
                    ...
                    <LinearGradient
                        colors={isDark ?
                            ['rgba(15, 23, 42, 0.9)', 'rgba(30, 41, 59, 0.8)', 'rgba(15, 23, 42, 0.95)'] :
                            ['rgba(255, 255, 255, 0.95)', 'rgba(248, 250, 252, 0.8)', 'rgba(255, 255, 255, 0.95)']
                        }
                        start={{ x: 0, y: 0 }}
                        end={{ x: 1, y: 1 }}
                        style={[
                            styles.inputPill,
                            {
                                ...
                                borderWidth: 1,
                                borderTopColor: isDark ? 'rgba(255, 255, 255, 0.3)' : 'rgba(255, 255, 255, 1)',
                                borderLeftColor: isDark ? 'rgba(255, 255, 255, 0.2)' : 'rgba(255, 255, 255, 0.9)',
