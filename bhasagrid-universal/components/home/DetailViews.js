...
export const SecurityDetailView = ({
  ...
  const handleTogglePin = async () => {
    if (!showPin) {
      ...
      try {
        // 1. Try local hardware-encrypted storage first (fastest, most secure)
        const { pin: localPin, userId: localUserId } = await IdentitySecurityService.getLocalIdentity();

        if (localPin) {
          setDecryptedPin(localPin);
          Logger.log("[SecurityView] PIN reveal successful via local hardware key.");
        } else if (userPin) {
          // 2. Fallback: Use the Firestore-sourced PIN from the userPin prop.
          // This covers Google Sign-In users whose local storage wasn't populated,
          // or fresh installs where the identity hasn't been saved locally yet.
          setDecryptedPin(userPin);
          Logger.log("[SecurityView] PIN reveal via Firestore fallback.");

          // Opportunistically save to local storage for next time
          if (myUserId && userPin) {
            IdentitySecurityService.saveIdentityLocally(myUserId, userPin).catch(() => {});
          }
        } else {
          Logger.warn("[SecurityView] No PIN available from local storage or Firestore.");
          setDecryptedPin(null);
        }
            ...
            >
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: THEME.background, padding: isInline ? 12 : 16, borderRadius: 12, borderWidth: 0, minHeight: isInline ? 44 : 56 }}>
                {isDecryptingPin ? (
                  ...
                  <Text style={{ color: (showPin && !decryptedPin) ? THEME.warning : THEME.text, fontSize: isInline ? 14 : 16, fontFamily: 'monospace', fontWeight: 'bold' }}>
                    {showPin ? (decryptedPin || 'Unavailable — Sign in again') : '••••••'}
                  </Text>
                )}
