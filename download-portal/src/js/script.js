                ...
                >
                    ...
                    <div style={{
                        ...
                        <div id="nav-logo-landing-pad" style={{
                            ...
                            <img src="assets/BhasaGrid-Logo.webp" alt="BhasaGrid Logo" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                        </div>
            ...
            <div className={`mobile-nav-drawer ${menuOpen ? 'open' : ''}`} style={{ zIndex: 1500 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', padding: '0 0.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <img src="assets/BhasaGrid-Logo.webp" alt="Logo" style={{ width: '28px', height: '28px' }} />
                        <span style={{ fontSize: '1.1rem', fontWeight: 700, color: 'white' }}>BhasaGrid</span>
                    </div>
                ...
                <div style={{ width: '100%' }}>
                    <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
                        ...
                        <p style={{ color: 'var(--text-secondary)', fontSize: '1.3rem', maxWidth: '700px', margin: '0 auto' }}>
                            Experience the full power of BhasaGrid directly in your browser. Perfect for shared environments or quick access on any machine.
                        </p>
                    </div>
...
window.downloadFile = async function (platform) {
    try {
        ...
        if (platform === 'windows') {
            url = (config && config.downloadUrl) || "downloads/BhasaGrid_Setup.zip";
        } else if (platform === 'playstore') {
            url = (config && config.androidUrl) || "https://your-storage.com/cipherplay.apk";
            isExternal = true;
        } else if (platform === 'macos') {
            url = (config && config.macUrl) || "https://apps.apple.com/app/bhasagrid";
            isExternal = true;
        } else if (platform === 'linux') {
            url = (config && config.linuxUrl) || "downloads/BhasaGrid_Linux.tar.gz";
        }
        ...
        if (isExternal) {
            ...
            const fileName = url.split('/').pop() || 'BhasaGrid_Setup.zip';
            const link = document.createElement('a');
            link.href = url;
...
window.launchWebApp = async function () {
    const fallbackUrl = 'https://bhasagrid-bc8ce.web.app/';
    try {
